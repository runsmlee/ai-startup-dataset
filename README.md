...
> **Scope, up front:** This is a *curated sample*, not a global census of every AI startup. Treat country and category **counts** as a view of what we track, not as market-share truth. The most reliable signals are structural (funding concentration, stage mix), which track with broader venture data.
...
Companies are sourced from public funding disclosures, launch platforms, and news coverage, then verified and categorized editorially — a curated set, not a complete census. Funding ranges reflect **disclosed amounts only**, in USD. Corrections welcome — each profile page has a "Report Inaccuracy" flow, or open an issue here.
